...
void sig_handler(int sig_num, siginfo_t *info, void *args) {
  ...
  if (g_repeats == 1) {
    printf("Arguments of function sa_sighandler are:\nsiginfo_t is a structure "
           "of:\n");
    printf("  si_signo = %d\n", info->si_signo);
    printf("  si_errno = %d\n", info->si_errno);
    printf("  si_code = %d\n", info->si_code);
    ...
    ucontext_t ucontext = *(ucontext_t *)args;
    printf("\nucontext_t is a struct of:\n");
...
int main() {
  ...
  printf("oldact is a struct sigaction of:\n");
  printf("  pointer to function sa_action = %p\n", oldact.sa_sigaction);
  sigset_t samask = oldact.sa_mask;
  unsigned int *ptr = (int *)&samask;
  printf("  sigset_t sa_mask = %#x | %#x\n", *ptr, *(ptr + 1));
  printf("  sigset_t sa_flags = %lld\n", (long long)oldact.sa_flags);
  while (g_repeats) {
    sleep(1);
  }
